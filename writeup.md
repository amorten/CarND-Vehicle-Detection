...
**Note: when investigating the above images, it may seem that the white car is not detected. In fact, it is being detected by a single window, but because the threshold is set at 0.7, an average number of 0.7 detections over 10 frames is require. So, the heatmap averaging will not detect the white car until after about 7 frames. In fact, you can see it begins to be detected in the sixth frame. In a longer running video with prior history, the white car will be detected earlier.**
